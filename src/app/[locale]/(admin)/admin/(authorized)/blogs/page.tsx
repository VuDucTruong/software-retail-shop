...
import { Blog, Category, CategoryCreate } from "@/api";
import {
  ColumnDef,
  PaginationState,
  SortingState,
...
export default function BlogManagementPage() {
  ...
  const [status, lastAction, error, blogs, getBlogs, queryParams, deleteBlogs] =
    useBlogStore(
      useShallow((state) => [
        state.status,
        state.lastAction,
  ...
  const cols: ColumnDef<Blog>[] = [
    ...
    },
    {
      ...
      header: "Thể loại",
      cell: ({ row }) => {
        return row.original.genres.join(", ");
      },
    },
    {
    ...
    <Card>
      ...
      <CardContent>
        <CommmonDataTable
          ...
          columns={cols}
          data={blogs?.data ?? []}
          totalCount={blogs?.totalInstances ?? 0}
          pageCount={blogs?.totalPages ?? 0}
