...
const genCols = ({
  ...
  return [
    ...
    {
      accessorKey: "amount",
      header: ({ column }) => <SortingHeader column={column} title={t("Amount")} />,
      cell: ({ row }) => {
    ...
    },
    {
      ...
      header: ({ column }) => <SortingHeader column={column} title={t("Time")} />,
      cell: ({ row }) => {
        ...
        return (
          <>
            <div className="flex items-end gap-2">
              ...
              {convertStatus(row.original.orderStatus ?? "PENDING") !==
              "failed_mail" ? null : (
                <CommonToolTip content={t("resend_mail")}>
                  <OrderResendMailDialogContent
                    email={row.original.sentMail}
                    onSubmit={(email) =>
                      handleResendMail(row.original.id, email ?? "")
                    }
                  />
                </CommonToolTip>
              )}
...
export default function TransactionMangementPage() {
  ...
  function onSearchAndStatusesDebounced(
    selectedStatuses: (number | string)[],
    search: string
  ) {
    const firstCleaned = selectedStatuses.filter(
      (s) => s !== ORDER_STATUS_NONE_ID
    );

    if (!StringUtils.hasLength(search) && CollectionUtils.isEmpty(firstCleaned))
      return;
    const finalCleaned =
      firstCleaned.length === OrderStatusSchema.options.length
        ? []
        : firstCleaned;
        ...
        <CommmonDataTable
          searchComponent={
            <SearchWithDropDown
              menus={{
                items: [
                  { id: ORDER_STATUS_NONE_ID, name: "All" },
