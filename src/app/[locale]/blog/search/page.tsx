...
export default function BlogSearchPage() {
  ...
  const [blogs, status, error] = BlogMany.useStoreLight(
    useShallow((s) => [s.blogs, s.status, s.error])
  );
  const [genre1s, genre2s] = GenreDomain.useStore(
    useShallow((s) => [s.genre1s, s.genre2s])
  );

  const searchTitle =
    search.length > 0
      ? t("search_results_for_x", { x: search })
      : t("search_results");
  return (
    <div className="main-container flex flex-col gap-4 !pt-10">
      ...
      <StatusDependentRenderer
        status={status}
        error={error}
        altError={<SearchNotFound className="col-span-4" objectName={t("products")}/>}
      >
        <div className="grid grid-cols-4 gap-3 py-4">
          {blogs.map((b) => {
            const blogGenre2s = genre2s
              .filter((g2) => b.genre2Ids.some((g2Id) => g2Id === g2.id))
              .map((g2) => g2.name);
            return (
